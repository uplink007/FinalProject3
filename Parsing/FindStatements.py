...
def parse2file(sentences, DorS, url, index):
    if os.path.isfile('DefinitionsFromStatementJSON.txt'):
        f1='a'
    else:
        f1='w'
    if os.path.isfile('DefinitionsFromStatementJSONURLs.txt'):
        f2='a'
    else:
        f2='w'
    ...
    if os.path.isfile('StatementsURLs.txt'):
        ...
        f4='w'
    filename =  open('DefinitionsFromStatementJSON.txt', f1, encoding = 'utf-8')
    filename1 =  open('DefinitionsFromStatementJSONURLs.txt', f2, encoding = 'utf-8')
...
fileName.close()
res = Create_URL_List(OpenJson('data\\Stats.json'))
for indx, line in enumerate(res):
    string_ = url_Parse_double(line)
